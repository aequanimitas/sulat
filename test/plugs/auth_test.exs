defmodule Sulat.AuthTest do
  ...
  test "authentication halts when :active_user does not exist", %{conn: conn} do
    # authenticate_user expects key :active_user to exist
    # since auth is still not wired with the routes, assign :active_user to nil here
    conn = conn |> assign(:active_user, nil) |> Sulat.Auth.authenticate_user([])
    assert conn.halted
  end

  test "authentication proceeds when :active_user exists", %{conn: conn} do
    conn = conn |> assign(:active_user, %Sulat.User{}) |> Sulat.Auth.authenticate_user([])
    refute conn.halted()
  end
  ...
  test "logging in", %{conn: conn} do
    ...
    assert get_session(next_conn, :active_user_id) == 100
  end

  test "logging out", %{conn: conn} do
    login_conn = conn
                 |> put_session(:active_user_id, 100)
                 |> Sulat.Auth.logout()
                 |> send_resp(:ok, "")
    next_conn = get(login_conn, "/")
    refute get_session(next_conn, :active_user_id)
  end

  test "put Sulat.User from session into assigns", %{conn: conn} do
    user = create_user()
    conn = conn |> put_session(:active_user_id, user.id) |> Sulat.Auth.call(Sulat.Repo)
    assert user.id == conn.assigns.active_user.id
  end

  test "call with no session yields to a nil :active_user", %{conn: conn} do
    conn = Sulat.Auth.call(conn, Sulat.Repo)
    assert nil == conn.assigns.active_user
  end
  ...
  test "testing whole pipeline", %{conn: conn} do
    ...
    refute get_session(conn, :active_user_id)
  end
  ...
  test "dropping session?", %{conn: conn} do
    ...
    assert get_session(login_conn, :active_user_id) == nil
  end
  ...
  test "login_with_username_password", %{conn: conn} do
    ...
    assert login_conn.assigns.active_user.id == user.id
  end
